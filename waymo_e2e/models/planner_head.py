...
class LightTrajectoryHead(nn.Module):
    """
    Lightweight planner: optional small Transformer over fused tokens, attention pooling,
    then MLP → (T, xyz). No pretrained LM.
    """
    ...
    def __init__(
        ...
        enc_layer = nn.TransformerEncoderLayer(
            d_model=d_model,
            nhead=nhead,
            dim_feedforward=max(hidden_dim * 2, d_model * 2),
            dropout=dropout,
            activation="gelu",
            batch_first=True,
            norm_first=True,
    ...
    def forward(self, bev, time_tokens, pose_emb, route_emb, additional_input=None):
        parts = [bev, time_tokens, pose_emb, route_emb]
        if additional_input is not None:
            parts.append(additional_input)
        x = torch.cat(parts, dim=1)
        x = self.encoder(x)
        w = torch.softmax(self.attn_pool(x).squeeze(-1), dim=-1)
        pooled = (x * w.unsqueeze(-1)).sum(dim=1)
        pooled = self.dropout(pooled)
        pred = self.mlp(pooled).view(-1, self.num_waypoints, 3)
...
class PlannerHead4D(nn.Module):
    """T5 encoder over concatenated tokens + linear waypoint head; optional depth BEV branch."""
